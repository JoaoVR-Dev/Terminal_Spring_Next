...
export default function DeleteContainer() {
  ...
  const deleteContainer = async () => {
    const response = await axios.delete(
      'http://localhost:8080/container/delete/' + containerController.id
    );
    setContainerController({
      id: 0,
      numberContainer: '',
  ...
  return (
    <div className={`
    flex-initial flex-col mr-10 border-8 border-white border-solid rounded-2xl space-y-2 h-auto p-4 w-72
    `}>
      <h1 className='titulosconc'>Deletar</h1>
      <div>
      <label className={`
      sr-only
      `}></label>
      ...
      <Button texto='Deletar' onclick={() => deleteContainer()}/>
    </div>
  )
}
